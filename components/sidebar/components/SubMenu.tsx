...
import { StyledImage } from "layout/sidebar/style";
...
export const SubMenuFR: React.ForwardRefRenderFunction<
  HTMLLIElement,
  SubMenuProps
> = (
  {
  ...
  React.useEffect(() => {
    const handleDocumentClick = (event: MouseEvent) => {
      if (
        ...
        (!SubMenuContentRef.current?.contains(event.target as Node) &&
          openWhenCollapsed)
      ) {
        setOpenWhenCollapsed(false);
      }
